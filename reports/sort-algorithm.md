...
| 2 | build the candidate set | L0 **attention** reads all input keys **at the SEP position** and assembles the present-key set there (≈ a sum of key embeddings). `SEP` holds a readable-but-lossy copy; the causally-used identities are **distributed** across the block-0 residual, not a swappable `SEP` object. The L0 MLP is causally essential for keys but does *not* build this set code — its precise key-role is unlocalized | `SEP` residual predicted by the present-key multi-hot at R²≈0.84 (≈emb-sum 0.81); per-key presence AUC 0.88 (non-uniform: mid-range keys ~0.75); the set code is unchanged before/after the L0 MLP yet ablating the L0 MLP collapses key accuracy 1.00→0.20; transplanting `SEP` alone installs a new set only at 0.08 (full block-0 transplant 1.0) | **causal + probe** (`walkthrough`, `agent_sorthead`, `riddle_menu`, `riddle_causal`) |
